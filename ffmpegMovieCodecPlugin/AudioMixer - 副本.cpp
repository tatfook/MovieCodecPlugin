...
void AudioMixer::InitFilerGraph(int chanels)
{
	...
	//	return;// note: mem leak
	//}
	//AVFilterContext* pDelayFilterContext;
	//// Beware of this filter's args
	//std::string args;
	//char digits[16];
	//for (int i = 0; i < m_Audios.size(); ++i){
	//	memset(digits, 0, 16);
	//	itoa(m_Audios[i].m_fStartTime, digits, 10);
	//	args.append(digits);
	//	args += "|";
	//}
	//// Remove the last "|" symbol.
	//args.resize(args.size()-1);
	//avfilter_graph_create_filter(&pDelayFilterContext, pDelayFilter, "adelay",
	...
	for (int i = 0; i < m_pBufferFilterContexts.size(); ++i) {
		ret = avfilter_link(m_pBufferFilterContexts[i], 0, pMixFilterContext, 0);
		if (ret < 0) {
			OUTPUT_LOG("Could not link the buffer filt to mix filter!\n");
			return;// note: mem leak
		}
	}

	//ret = avfilter_link(pDelayFilterContext, 0, pMixFilterContext, 0);
	//if (ret < 0) {
	//	OUTPUT_LOG("Could not link the delay filter to mix filter!\n");
	//	return;// note: mem leak
	//}
